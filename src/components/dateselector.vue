            ...
            <div class="date-selector-container date-selector-container-up">
                <div class="date-selector-btn-box">
                    ...
                    <div v-if="type==1" class="date-selector-tab-box">
                        <div @click.stop="showYear()" class="date-selector-tab"
                             :class="isYear?'date-selector-tab-active': ''"
                        >年月日</div>
                        <div 
                            @click.stop="showHour()"
                            class="date-selector-tab"
                            :class="!isYear?'date-selector-tab-active': ''">时分</div>
					</div>
                    <div @click.stop='makeSureData' class="date-selector-btn">提交</div>
				</div>
				<div v-if="type == 0" class="date-selector-content">
                    <selector-list v-for="(item,index) in htmlList" :key="index"
                        :style="styleObj"
    ...
    export default {
        data(){
            return {
                ...
                idxArr: [],
                endTime: [],
                recentTime: [date.getFullYear(), date.getMonth() + 1, date.getDate(), date.getHours(), date.getMinutes()],
                liHeight: 40,
                resultArr: [],  //  最终返回的数据
                maxHeight: [],
                distance: [],
            ...
            checkParam() {
                ...
                for(let i = 0; i < this.param.length; i++){
                    if (/(\d)+\-(\d)+/.test(this.param[i])) {
                        let arr = this.param[i].split('-')
                        let arrHtml = [];
                        const minMax = fullDose
                                       ?Math.max(Number(arr[0]),this.recent_time[i])
                                       :Number(arr[0]);
                        arrHtml.push(minMax,Number(arr[1]));
                        this.htmlList[i] = arrHtml
                        idxArr.push(i)
...
.date-selector-content .date-selector-line:after,.date-selector-content .date-selector-line:before{
    content: '';
    position: absolute;
    display: block;
    width: 100%;
    height: 1px;
    background: -webkit-linear-gradient(#dbdbdb, #161616 , #dbdbdb); /* Safari 5.1 - 6.0 */
    background: -o-linear-gradient(#dbdbdb, #161616 , #dbdbdb); /* Opera 11.1 - 12.0 */
    background: -moz-linear-gradient(#dbdbdb, #161616 , #dbdbdb); /* Firefox 3.6 - 15 */
