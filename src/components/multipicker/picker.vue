<template>
    <div>
        <ul ref="arrIdx" @touchstart.prevent="touch($event)" @touchmove.prevent="touch($event)" @touchend.prevent="touch($event)">
            <li></li>
            <li></li>
            <li v-for="(item,index) in areaData" :key="index">{{item | validArr}}</li>
    ...
    export default {
        ...
        methods: {
            ...
            setTransform($picker,val = 0){
                $picker.style.transform        = `translate3d(0,${val}px, 0)`;
                $picker.style.webkitTransform  = `translate3d(0,${val}px, 0)`;
            },
