	...
	<div id="coupon" v-cloak>
		<div class="header">
			<template v-for="(title,index) in title">
				<div @click="changeTitle(index)" :key='index' ><span>{{title}}</span>
		...
		<div class="content">
			...
			<template v-for="(c,index) in showList">
				<div class="discount" :key="index" @click="openCoupponDetail(c)">
					...
					<template v-if="c.createTime > time && index == 0">
						<div class="jiao">
							新
						</div>
...
export default{
	...
	methods:{
        ...
        async getUserInfo(){
        	let data = await http.getUserInfo({data:{//请求登录接口
        		shopId:shopId,
        		type:4,
        		from:0
        	}});
        ...
        getList() {
        	http.getCouponList({data:{//获取优惠券
        		shopId: shopId
        	}},true,true).then(res=>{
				this.list = res.data;//全部优惠券
				// 获取返回优惠券时间的零点时间戳
				var times = utils.getZero(new Date(res.time * 1000)) / 1000;
				this.time = times;
				this.success = true;
		...
		getShowList: function() {//筛选优惠券
			var arr = utils.deepCopy(this.list);//拷贝优惠券的数据
			var useList = [],//未使用的优惠券
			    usedList = [],//已使用的优惠券
			    expireList = [];//已过期的优惠券数组
			for (var i = 0; i < arr.length; i++) {
				if (!this.checkExpire(arr[i].endTime)) {
					arr[i].status = -1;
					expireList.push(arr[i]);
				} else if (arr[i].status == 0 && this.checkExpire(arr[i].endTime)) {
					useList.push(arr[i]);
				} else if (arr[i].status == 1 && this.checkExpire(arr[i].endTime)) {
					usedList.push(arr[i]);
				}
			}
			//根据创建时间依次排序
			this.useList = utils.sortByAll(useList,"createTime",true);
			this.usedList = utils.sortByAll(usedList,"createTime",true);
			this.expireList = utils.sortByAll(expireList,"createTime",true);
			this.changeTitle(0);
		},
		changeTitle: function(index) {//切换标题title
			this.indexSelect = index;
			if (this.indexSelect == 0) {
				this.showList = this.useList;
			} else if (this.indexSelect == 1) {
				this.showList = this.usedList;
			} else {
				this.showList = this.expireList;
			}
		},
		...
		getClass: function(num) {//根据类名获取样式
			if (this.indexSelect == 0) {
		...
		getText: function(num) {//根据类型判断是哪种类型的优惠券
			if (num == 0 || num == 1 || num == 2) {
				return '减';
			} else if (num == 3 || num == 4) {
				return "折"
...
.dis-name
{
	...
	color: #626364; /*background-color: yellow;*/
}
.dis-desc
{
	font-size: .42rem; 	/*background-color: #ADFF2F;*/
...
.dis-date
{
	...
	color: #626364;/*background-color: #FFA500;*/
}
/*已使用*/
.dis-used
{
