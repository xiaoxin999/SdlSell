<template>
  <div id="packages" v-cloak>
    <div class="package">
        <div class="package-tit">{{packageTitle}}<span class="previousPage fl" @click="openOrder"></span></div>
         <!-- 可选套餐 -->
                                    ...
                                    <span class="fl attr-name">
                                        <template v-for="(attrLsit,h) in attr.attrs">
                                            <template v-for="(options,g) in attrLsit.options" v-if="options.attrStatu">
                                                <p class="fl" :key="h+'_'+g+'_1'">{{options.name}}</p> <p :key="h+'_'+g+'_2'" class="fl attr_hido">/</p>
                                            </template>
                                        </template>
                                    </span>
                            ...
                            <template v-for="(attr,k) in goods.attrList">
                                <div class="package-attr" v-if="goods.isAttr && attr.attrs && attr.attrs.length >= 1" :key="'pack_attr_'+k">
                                    <span class="fl attr-name">
                                        <template v-for="(attrLsit,h) in attr.attrs">
                                            <template v-for="(options,g) in attrLsit.options" v-if="options.attrStatu" >
                                                <p class="fl" :key="'attr_p1_'+h+'_'+g">{{options.name}}</p> <p class="fl attr_hido" :key="'attr_p2_'+h+'_'+g">/</p>
                                            </template>
                                        </template>
                                    </span>
                                    <span class="fl attr-name shoName" v-if="attr.showAttr">无口味</span>
	...
	<transition name="fade">
		<attr-win 
			v-if="isAttrWin"
			:packagesAttr = "packagesAttr"
			@isChangeAttr = "isChangeAttr"
			:Multiselect = "Multiselect"
			@changePackagesAttr = "changePackagesAttr"   
...
export default {
  ...
  methods: {
    ...
    toogleSub: function(i, subDetail) {
      //切换副标题与商品
      var subId = subDetail.id;
      var arr = this.packageSub;
    ...
    addPackageNum: function(j, goods, type) {
      //选择可选套餐商品数量
      var numMax = this.packageGoods[0].totalNum;
      // 根据type值判断为添加还是删除
      if (type == "add") {
        if(goods.num >= goods.goodsNum){
          var content = goods.goodsName + '数量仅剩' + goods.goodsNum + '份';
          this.$store.commit("setWin",{content: content});
          return false;
        }
        var packageGoods = this.packageGoods[0].goods;
        var num = 0;
        for (var k = 0; k < packageGoods.length; k++) {
    ...
    conservationPackages: function() {
		...
		if (this.Multiselect) {
			//可选套餐
			var arr = this.packages.packageTag;
			var arr1 = this.packageSub;
			var numMax = 0; //套餐可选总数量
			var num = 0; //商品已选数量
			for (var i = 0; i < arr.length; i++) {
			...
			//固定套餐
			var arr = this.packages.packageTag;
			var arr1 = this.fixedPackages;
			for (var k = 0; k < arr1.length; k++) {
				this.packages.packageKey += arr1[k].id;
				for (var h = 0; h < arr1[k].goods.length; h++) {
		...
		var pl = utils.deepCopy(this.packages);
		for (var i = 0; i < pl.packageTag.length; i++) {
			var gl = [];
			for (var j = 0; j < pl.packageTag[i].goods.length; j++) {
				if (pl.packageTag[i].goods[j].attrList.length > 0) {
					for (
					var k = 0;
					k < pl.packageTag[i].goods[j].attrList.length;
					k++
					) {
					var g = utils.deepCopy(pl.packageTag[i].goods[j]);
		...
		var sendP = utils.deepCopy(pl);
		// 回到主页面
		sendP.index = orderData.packageIndexObj[sendP.id];
  ...
  async mounted() {
    ...
    var packagesList = storage.session("searchPackage"); //获取传递过来的套餐数据
    if (packagesList) {
      var tagPackages = packagesList.packageTag;
      var a = 1;
      for (var i = 0; i < tagPackages.length; i++) {
        tagPackages[i].isOn = false;
        tagPackages[0].isOn = true;
        var goods = tagPackages[i].goods;
        for (var j = 0; j < goods.length; j++) {
          // goods[j].isAttr = false;
          goods[j].num = "0";
          // goods[j].attr = [];
          goods[j].attrList = [];
          // if (utils.isEmptyObject(orderData.getTasteList(goods[j].id))) {
          //   goods[j].attr = [];
          // } else {
          //   goods[j].attr = orderData.getTasteList(goods[j].id);
          //   var attrs = goods[j].attr;
          //   for (var k = 0; k < attrs.length; k++) {
          //     for (var h = 0; h < attrs[k].options.length; h++) {
  ...
  components: {
    "attr-win": () => import(/* webpackChunkName: "arrt_win" */ "./attr_win")
  }
};
</script>
<style scoped>
