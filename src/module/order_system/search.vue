...
<div class="goods-top">
    <div class="goods-box" v-for="(goods,i) in goodsList" @click="showGoodsDetail(goods)" :key="i+'_goods'">
        ...
        <div class="goods-price">
        	<span :class="goods.isVip == '1' && isShowVipPrice ? 'fl' : '' ">¥{{goods.price}}</span>
			<span v-if="goods.isVip == '1' && isShowVipPrice" class="vipPrice fl" style="margin-left: 0.29333rem;">会员价&nbsp;¥{{goods.vipPrice}}</span>
        </div>
        <div class="goods-num" v-if="checkIsIdentical(goods) > 0">{{checkIsIdentical(goods)}}</div>
  ...
  watch: {
    ...
    parseOk(newV, oldV) {
      if(newV) {
        ...
        this.packages = orderData.others.packageArr.package;   
      }
    }
  },
  components: {
