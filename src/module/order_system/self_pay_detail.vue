            ...
            <div class="orderDetail">
                ...
                <div class="detailList" ref='detailList'>
                    ...
                    <div class="detaList" ref = 'detaList' v-else>
                        <!-- 打折菜 -->
                        <div class="details" v-for="(item,index) in discountList" :key='index'>
                            <div class="dishNames">
                        ...
                        <!-- 赠送菜品 单品 -->
                        <div class="details" v-for="(item,index) in giftitemList" :key='index'>
                            <div class="dishNames">
                                <p class="present"></p>
                                <p class="dishName">{{item.goodsName}}<span v-if="item.taste">({{item.taste}})</span></p>
                            </div>
                        ...
                        <!-- 赠送的套餐 -->
                        <section v-for="(item,index) in giftPackage" :key='index'>
                            <div class="details">
                                <div class="dishNames">
                                    <p class="present"></p>
                        ...
                        <!-- 打折的套餐 -->
                        <section v-for="(item,index) in dispackageList" :key='index'>
                            <div class="details">
                                <div class="dishNames">
                                    <p class="discountIcon"></p>
                                    <p class="dishName">(套餐){{item.packageName}}</p>
        ...
        methods:{
            ...
            async checkTable() {
                ...
                if (!utils.isEmptyObject(data.orderDetail.goods)) {
                    var arr = data.orderDetail.goods;
                    //第一层循环
                ...
                if (!utils.isEmptyObject(data.orderDetail.package)) {
                    var brr = data.orderDetail.package;
                    //第一层循环
                    ...
                    //第二层循环
                    for (var j = 0; j < brr[i].goods.length; j++) {
                        brr[i].goods[j].taste = ''; //口味
                        brr[i].goods[j].num = Number(brr[i].goods[j].num); //数量
                        //第三层循环
                        for (var k = 0; k < brr[i].goods[j].attr.length; k++) {
                        //第四层循环
                        for (var l = 0; l < brr[i].goods[j].attr[k].name.length; l++) {
                ...
                this.$nextTick(function() {
                    ...
                    if (data.order.status == "4") {
                        this.$store.commit('setWin',{content:'该桌台不可用,请联系服务员清台..'})
                    }
                })
            },
            ...
            toOrder: function() {
                ...
                if (this.isTableStatus == "1") {
                    this.$store.commit('setWin',{content:'您有订单待确认,请联系服务员确认..'})
                    return false;
                }
                let orderInfo = {};
            ...
            async toPay() {
                ...
                let data = await http.payUrl({
                    data:{
                        shopId:shopId,
                        type: 6
                    }
                })
                window.location.href = data; //跳转到支付链接
...
#selfPay-detail .orderPay .payOrderBtn .onOrder
{
    ...
    border-radius: .13333rem;
    background: #f4ca73;
    background: -webkit-linear-gradient(left,#f4cf81 , #f5bf4f); /* Safari 5.1 - 6.0 */
    background:      -o-linear-gradient(right, #f4cf81, #f5bf4f); /* Opera 11.1 - 12.0 */
    background:    -moz-linear-gradient(right, #f4cf81, #f5bf4f); /* Firefox 3.6 - 15 */
    background:         linear-gradient(to right, #f4cf81 , #f5bf4f); /* 标准的语法 */
}
