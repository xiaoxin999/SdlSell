<template>
    <div class="pay-detail">
        ...
        <transition name="fade">
            <win 
                v-if="isPayWin && isCancel"
                :height = '"auto"'
                @winEvent="cancelReserve"
                :ok="cancel"
            >
        ...
        <!-- <transition name="fade">
            <div class="payWin" v-if="isPayWin">
                <div class="payTip" v-if="isCancel">
                    ...
                    <div class="payFooter">
                        <a href="javascript:void(0)" class="confirm" @click="cancelReserve">确认退款</a>
                        <a href="javascript:void(0)" class="cancel" @click="winHandle()">取消</a>
                    </div>
                </div>
                <div class="payTip" v-else>
                    <div class="payConcent">
                        <div class="payTit">支付规则</div>
                        <div class="payBox">
                            <ul>
    ...
    export default {
        ...
        methods: {
            async getReserveDetail(roid){
                ...
                this.goodsList = res.data.goodsData.goods;
                this.packagesList = res.data.goodsData.package;
                this.isEdit = true;
			...
			async deleteOrder(){
                let res = await http.deleteReserve({
					data: {
						shopId : this.shopId,
                        roid : this.roid
					}
            ...
            async createOrderReserve(){
                ...
                reserveTime = Math.round(reserveTime.getTime() / 1000);
                 ...
                 // 验证信息
                if(!global.checkData({
					name : {cond: '$$.length > 0',pro: '请填写姓名..'},
					num : {reg: /\d{1,3}/g,pro: '请输入人数..'},
					phone : {reg: /^1\d{10}/g,pro: '请输入电话..'},
				},this)){
                    return false
				}
				let res = await http.createOrderReserve({
					data: {
						...
						phone : this.phone,
						remark : this.remark,
	                    //openId : 'oZjj-wZ1cIOIFwZ_kO3HDTrSPQ1U',
						openId : this.openId,
						data : this.goodsInfo
					}
				})
				this.roid = res.orderId;
				this.data = res;
				this.callpay();
			},
			callpay(){
            ...
            async cancelReserve(type){
                if(type == 'ok'){
                    let res = await http.cancelReserve({
                        data: {
                            shopId: this.shopId,
                            roid : this.roid
                        }
            ...
            async getReserveConf(){
                ...
                if(this.status == '5'){
                    this.discount = '100';
                    this.actualRefund = this.sumPrice;
                    this.brokerage = '0';
                }
            ...
            getDate(times,num){
                ...
                var dates = new Date(startTime); //时间副本 用于获取times分钟
                    dates.setMinutes(dates.getMinutes()+Number(num));
                return Math.round(dates.getTime()/1000);
            },
            getTime(startTime,endTime){
                var date1 = new Date(startTime*1000);  //开始时间
                var date2 = new Date(endTime*1000);     //结束时间
        ...
        async mounted(){
            ...
            // let order = storage.session('paySend');
            var reservationInfo = storage.session('reservationInfo');
            ...
            if(process.env.ASSET_PATH){
                this.roid = this.$route.query.roid;
            }else{
                this.roid = storage.session('roid')
            }
            if(this.roid){
                this.getReserveDetail(this.roid) 
            }else{
				var goods = storage.session('goods');
				if(!utils.isEmptyObject(goods)){
					var goodsFilter = goods.filter(function(item,index,array){
							item.price = (parseFloat(item.price) * parseFloat(item.num)).toFixed(2);
							var arr = item.attr;
								if(!utils.isEmptyObject(arr)){
									arr.filter(function(item1){
									var options = item1.options;
...
.payWin {
 ...
 .payFooter {
      ...
      line-height: 1.33333rem;
      font-size: 0; }
     .payFooter .cancel, .payFooter .confirm {
        width: 50%;
        height: 1.33333rem;
        ...
        color: #FFFFFF;
        text-align: center; }
     .payFooter .cancel:hover, .payFooter .confirm:hover {
          text-decoration: none; }
     .payFooter .cancel {
...
/*# sourceMappingURL=paydetails.css.map */
