        ...
        <div class="member-cards-content">
            <div class="member-cards-listBox">
                ...
                <ul class="member-cards-list">
                    <li class="member-cards-item" v-for="(item,index) in membercard.card" :key="index">
...
<script>
    ...
    export default {
        ...
        methods: {
            async getCardsAndList(){
                let res = await http.getMemberCardAndCouponList({
                    data: {
                        shopId: this.shopId
                    }
                })
                storage.session("cardList", res)
                this.couponTotal = res.couponTotal;
                this.qrcodeUrl = global.host + 'common/qrcode?format=json&text=' + res + '&' + 'size=10';
                this.membercard = res;
            },
