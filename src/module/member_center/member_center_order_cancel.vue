<template>
	...
	<div class="order-header">
		<div class="header-logo">
			<div class="logo-img" :style="{backgroundImage: 'url('+ getLogo(userDate.shopList,userDate.id) +')'}">
			</div>
		</div>
		...
		<div class="cancel-handle">
			<template v-if="isTimes">
				...
				<a href="javascript:void(0)" class="payoff" @click="getUrl">去支付</a>
			</template>
			<template v-else>
        ...
        methods : {
    		async showWctDetail() {
                ...
                let data = await http.showOrderDetailPost({data:{
                    oid: storage.session('oids'),
					shopId: this.userDate.id == undefined ? this.userDate.shopId : this.userDate.id,
			...
			getLogo : function(list,shopId){
				// 获取logo地址
				var i,logoImage;
				for(i=0;i<list.length;i++){
					if(list[i].id == shopId){
						logoImage = list[i].logoImage;
					}
				}
				return global.getImgUrl() + logoImage;
			},
			...
			async getUrl(){
                // 获取详情url
                let data = await http.payUrl({data:{
                    shopId : this.userDate.id == undefined ? this.userDate.shopId : this.userDate.id,
					type : 2
                }})
			...
			openOrder : function(){
				...
				for(var i=0;i<obj.shopList.length;i++){
					if(obj.shopList[i].id == this.userDate.id){
						obj.shopName = obj.shopList[i].name;
					}
				}
